...
const command: ICommand = {
  ...
  async execute(message: Message, _con: Client, args?: string[]) {
    console.log(
      `Command randomnumber started by user ${
    ...
    if (isNaN(numMin) || isNaN(numMax)) {
      // if either the minimum or the maximum is not a number
      try {
        console.log(
          "Invalid value given for minimum or maximum value. Stopping execution."
        );
        return await message.channel.send(
          "Invalid number for one or more arguments."
        );
      } catch (e) {
        console.log(
          `There was an error sending a message in the guild ${
    ...
    try {
      ...
      console.log(
        `Command randomnumber, started by ${
          message.member!.user.tag
