...
const command: ICommand = {
  ...
  async execute(message: Message | Channel, _con: Client, _args?: string[]) {
    if (message instanceof Message) {
      // if someone manually requested the start message
      console.log(
        `Command start started by user ${message.member!.user.tag} in guild ${
          message.guild!.name
        }.`
      );
      ...
      const outputEmbed = new MessageEmbed() // create a new embed for output
        ...
        .addFields(
          ...
          },
          {
            name: "Services",
            value:
              "To see a full list of the services Fulcrum offers and how to set them up, run f!services.",
          }
        );
      ...
      // if it was triggered automatically (like on joining a guil )
      console.log(
        `Ambient start command triggered in ${
          (message as TextChannel).guild.name
        }.`
      );
      ...
      try {
        ...
        console.log(
          `Ambient start message sequence completed successfully in ${
            (message as TextChannel).guild.name
          }.`
        );
      } catch (e) {
