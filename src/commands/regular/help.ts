...
const command: ICommand = {
  ...
  async execute(message: Message, _con: Client, args?: string[]) {
    ...
    const adminCommandFiles = await globPromise(
      `${__dirname}/../admin/**/*.{js,ts}`
    ); // identify command files
    const regularCommandFiles = await globPromise(
      `${__dirname}/../regular/**/*.{js,ts}`
    ); // identify command files
    const serviceCommandFiles = await globPromise(
      `${__dirname}/../service/**/*.{js,ts}`
    ); // identify command files
    ...
    if (args!.length > 0) {
      ...
      if (adminOutputEmbedText) {
        // check if there are actually admin commands to output
        outputEmbed.addField("Admin Commands", adminOutputEmbedText);
      }

      if (regularOutputEmbedText) {
        // check if there are actually regular commands to output
        outputEmbed.addField("Regular Commands", regularOutputEmbedText);
      }

      if (serviceOutputEmbedText) {
        // check if there are actually service commands to output
