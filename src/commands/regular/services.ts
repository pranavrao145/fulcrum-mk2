...
const command: ICommand = {
  ...
  async execute(message: Message, _con: Client, args?: string[]) {
    console.log(
      `Command services started by user ${message.member!.user.tag} in guild ${
        message.guild!.name
      }.`
    );
    ...
    if (args!.length > 0) {
      ...
      console.log(
        "Found argument supplied, attempting to find information for specific service."
      );

      const serviceMention = args!.shift()!.toLowerCase(); // get the service they want information about

      if (serviceList.has(serviceMention)) {
        // if the service is valid and help exists for it
      ...
      // if no service was supplied, list out the services
