...
const command: ICommand = {
  ...
  async execute(message: Message, _con: Client, args?: string[]) {
    console.log(
      `Command define started by user ${message.member!.user.tag} in guild ${
        message.guild!.name
      }.`
    );
    ...
    try {
      // attempt to get the definition from the api
      ...
      // if the api returns no response or an error
      console.log(
        `No defintions found for word ${word} or error getting definitions.`
      );
