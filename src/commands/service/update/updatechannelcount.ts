...
const command: ICommand = {
  slashCommand: new SlashCommandBuilder()
    .setName("updatechannelcount")
    .setDescription("Updates the channel count channel the current guild."),
  help: "Updates the channel count channel the current guild.",
  alias: ["ucc"],
  syntax: "f!updatechannelcount",
  async execute(message: Message | Guild, con: Client, _args?: string[]) {
    if (message instanceof Message) {
      // if a user is trying to manually update channel count
      console.log(
        `Command updatechannelcount started by user ${
      ...
      try {
        ...
        console.log(
          `Querying database to find channel count channel for guild ${
            message.guild!.name
          }.`
        );
        ...
        if (!row) {
          ...
          try {
            console.log(
              `Channel count channel ID for guild does not exist in database.`
            );
            return await message.channel.send(
              "Channel count channel not set up for this server! Run f!setup channelcount."
            );
          } catch (e) {
            console.log(
              `There was an error sending a message in the guild ${
        ...
        if (voiceChannel.name === nameToBeSet) {
          ...
          try {
            console.log(
              `Channel count channel for this guild is already updated.`
            );
            return await message.channel.send(
              "Channel count is already updated on this server!"
            );
          } catch (e) {
            console.log(
              `There was an error sending a message in the guild ${
        ...
        try {
          ...
          console.log(
            "Failed to upadate channel count channel. The error message is below:"
          );
          console.log(e);
      ...
      try {
        ...
        console.log(
          `Command updatechannelcount, started by ${
            message.member!.user.tag
      ...
      console.log(
        `Automatic channel count update triggered in ${message.name}.`
      );

      try {
        await timeout(1000); // wait for one second before querying the database
        console.log(
          `Querying database to find channel count channel for guild ${message.name}.`
        );
        const res = await con.query(
        ...
        if (!row) {
          // in the event a row does not exist
          console.log(
            `Channel count channel ID for guild does not exist in database. Stopping execution `
          );
          return;
        }
        ...
        if (voiceChannel.name === nameToBeSet) {
          // if the voice channel name is already correct, no need to change it
          console.log(
            `Channel count channel for this guild is already updated. Stopping execution.`
          );
          return;
        }
      ...
      console.log(
        `Automatic channel count update sequence completed successfully in ${message.name}.`
      );
    }
  },
};
