...
const command: ICommand = {
    ...
    alias: ['ud'],
    syntax: 'f!updatedate',
    async execute(message: Message | Discord.Client, con: Client, _args?: string[]) {
        if (message instanceof Message) { // if the user is trying to update manually
            ...
            if (!clientGuilds) { // if there are no guilds to update
                console.log('No guild found in which to update date. Stopping execution.');
                return;
            }
            ...
            for (const guild of clientGuilds) {
                ...
                try {
                    ...
                    const res = await con.query(`SELECT * FROM datechannel WHERE guildid = '${guild.id}'`); // find the id of the date channel for the guild 

                    const row = res.rows[0]; // get the first row from the database query result
                    let voiceChannel; // declare voice channel that may be initialized later if there is a match

                    if (!row) { // in the event a row does not exist
                        console.log(`Date channel ID for guild does not exist in database. Skipping over it.`);
                        continue;
                    }
                    ...
                    if (!voiceChannel) { // in the event the voice channel does not exist
                        console.log(`No voice channel found for the channel ID ${row.channelid} in the database. Skipping over guild.`);
                        continue;
                    }
                    ...
                    if (voiceChannel.name === nameToBeSet) { // if the voice channel name is already correct, no need to change i 
                        console.log(`Date channel for this guild is already updated. Skipping over it.`);
                        continue;
                    }
