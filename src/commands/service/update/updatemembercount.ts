...
const command: ICommand = {
  ...
  async execute(message: Message | Guild, con: Client, _args?: string[]) {
    if (message instanceof Message) {
      // if there is a message (a user triggered the command)
      console.log(
        `Command updatemembercount started by user ${
      ...
      try {
        ...
        console.log(
          `Querying database to find member count channel for guild ${
            message.guild!.name
          }.`
        );
        const res = await con.query(
          `SELECT * FROM membercountchannel WHERE guildid = '${
            message.guild!.id
          }'`
        ); // find the id of the member count channel for the guild of the message
        ...
        if (!row) {
          ...
          try {
            console.log(
              `Member count channel ID for guild does not exist in database.`
            );
            return await message.channel.send(
              "Member count channel not set up for this server! Run f!setup membercount."
            );
          } catch (e) {
            console.log(
              `There was an error sending a message in the guild ${
        ...
        if (voiceChannel.name === nameToBeSet) {
          ...
          try {
            console.log(
              `Member count channel for this guild is already updated.`
            );
            return await message.channel.send(
              "Member count is already updated on this server!"
            );
          } catch (e) {
            console.log(
              `There was an error sending a message in the guild ${
        ...
        try {
          await voiceChannel.setName(
            nameToBeSet,
            "Updated member count channel."
          ); // attempt to set the voice channel name to the right member count
          console.log("Member count channel updated successfully.");
          outputEmbed.addField("Status", "Success");
        } catch (e) {
          console.log(
            "Failed to upadate member count channel. The error message is below:"
          );
          console.log(e);
      ...
      try {
        ...
        console.log(
          `Command updatemembercount, started by ${
            message.member!.user.tag
      ...
      // if it is a member given (someone joined the guil )
      console.log(
        `Automatic member count update triggered in ${message.name}.`
      );

      try {
        await timeout(1000); // wait for one second before querying the database
        console.log(
          `Querying database to find member count channel for guild ${message.name}.`
        );
        const res = await con.query(
          `SELECT * FROM membercountchannel WHERE guildid = '${message.id}'`
        ); // find the id of the member count channel for the guild of the message
        ...
        if (!voiceChannel) {
          // in the event the voice channel does not exist
          console.log(
            `No voice channel found for the channel ID ${row.channelid} in the database. Stopping execution.`
          );
          return;
        }
        ...
        if (voiceChannel.name === nameToBeSet) {
          // if the voice channel name is already correct, no need to change it
          console.log(
            `Member count channel for this guild is already updated. Stopping execution.`
          );
          return;
        }
      ...
      console.log(
        `Automatic member count update sequence completed successfully in ${message.name}.`
      );
    }
  },
};
