...
const command: ICommand = {
    name: 'setup',
    description: 'Sets up the service specified. See f!services for a full list of services that Fulcrum offers.',
    alias: ['s'],
    ...
    async execute(message: Message, con: Client, args?: string[]) {
        ...
        switch (service) { // check for different services to setup
            case 'channelcount':
                serviceCommand = serviceCommands.find(c => c.name === 'setupchannelcount'); // get the channelcount command

                if (!serviceCommand) { // if the command does not exist
                    console.log('Service command setupchannelcount not found. Stopping execution.');
                    try {
            ...
            case 'date':
                ...
                if (!serviceCommand) { // if the command does not exist
                    console.log('Service command setupdate not found. Stopping execution.');
                    try {
                        return await message.channel.send('An internal error occured. Please try again later.');
                    } catch (e) {
                        console.log(`There was an error sending a message in the guild ${message.guild!.name}! The error message is below:`);
                ...
                break;
            case 'membercount':
                serviceCommand = serviceCommands.find(c => c.name === 'setupmembercount'); // get the channelcount command

                if (!serviceCommand) { // if the command does not exist
                    console.log('Service command setupmembercount not found. Stopping execution.');
                    try {
                        return await message.channel.send('An internal error occured. Please try again later.');
                    } catch (e) {
                        console.log(`There was an error sending a message in the guild ${message.guild!.name}! The error message is below:`);
            ...
            case 'vcroles':
                ...
                if (!serviceCommand) { // if the command does not exist
                    console.log('Service command setupvcroles not found. Stopping execution.');
                    try {
                        return await message.channel.send('An internal error occured. Please try again later.');
                    } catch (e) {
                        console.log(`There was an error sending a message in the guild ${message.guild!.name}! The error message is below:`);
