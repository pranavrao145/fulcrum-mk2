    ...
    async execute(message: Message, con: Client, args?: string[]) {
        ...
        try {
            ...
            if (row) { // if a row exists for the guild (update)
                ...
                query = `UPDATE channelcountchannel SET guildid   = '${message.guild!.id}', channelid = '${voiceChannel.id}' WHERE guildid = '${message.guild!.id}'` // update the existing channel count channel with the new one
            } else { // if a row does not exist for a guild (create)
                console.log('No entry found for guild in the database. Creating a new one.')
                query = `INSERT INTO channelcountchannel(guildid, channelid) VALUES ('${message.guild!.id}', '${voiceChannel.id}')`; // create a new row in database with id of channel supplied
            }

            try {
                await con.query(query); // try to query the database with the appropriate query
                console.log('Successfully updated channel count channel information in the database.')
        ...
        try { // send output embed with information about the command's success
            ...
            console.log(`Command setupchannelcount, started by ${message.member!.user.tag}, terminated successfully in ${message.guild!.name}.`);
        } catch (e) {
