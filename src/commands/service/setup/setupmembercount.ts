...
const command: ICommand = {
  slashCommand: new SlashCommandBuilder()
    ...
    .setDescription("Sets up Fulcrum's member count channel feature."),
  help: "Sets up Fulcrum's member count channel feature.",
  alias: ["smc"],
  syntax: "f!setupmembercount [voice channel role mention]",
  async execute(message: Message, con: Client, args?: string[]) {
    console.log(
      `Command setupmembercount started by user ${
    ...
    try {
      console.log(
        "Attempting to retrieve member count channel information from the database."
      );
      ...
      if (row) {
        ...
        query = `UPDATE membercountchannel SET guildid   = '${
          message.guild!.id
        }', channelid = '${voiceChannel.id}' WHERE guildid = '${
          message.guild!.id
        }'`; // update the existing member count channel with the new one
      } else {
        // if a row does not exist for a guild (create)
        console.log(
          "No entry found for guild in the database. Creating a new one."
        );
        query = `INSERT INTO membercountchannel(guildid, channelid) VALUES ('${
          message.guild!.id
      ...
      try {
        ...
        console.log(
          "Successfully updated member count channel information in the database."
        );
      ...
      const updateMemberCountCommand = serviceCommands.find(
        (c) => c.slashCommand.name === "updatemembercount"
      ); // get the updatemembercount command
      ...
      if (updateMemberCountCommand) {
        ...
        console.log(
          "Command updatemembercount not found. Skipping attempted updating of newly created member count channel."
        );
      }
    } catch (e) {
      console.log(
    ...
    try {
      ...
      console.log(
        `Command setupmembercount, started by ${
          message.member!.user.tag
