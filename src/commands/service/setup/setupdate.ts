...
const command: ICommand = {
    ...
    async execute(message: Message, con: Client, args?: string[]) {
        console.log(`Command setupdate started by user ${message.member!.user.tag} in guild ${message.guild!.name}.`);
        ...
        try {
            console.log('Attempting to retrieve date channel information from the database.')
            const res = await con.query(`SELECT * FROM datechannel WHERE guildid = '${message.guild!.id}'`); // see if the guild already has a channel ID in the database
            ...
            if (row) { // if a row exists for the guild (update)
                ...
                query = `UPDATE datechannel SET guildid   = '${message.guild!.id}', channelid = '${voiceChannel.id}' WHERE guildid = '${message.guild!.id}'` // update the existing date channel with the new one
            } else { // if a row does not exist for a guild (create)
                console.log('No entry found for guild in the database. Creating a new one.')
                query = `INSERT INTO datechannel(guildid, channelid) VALUES ('${message.guild!.id}', '${voiceChannel.id}')`; // create a new row in database with id of channel supplied
            }

            try {
                await con.query(query); // try to query the database with the appropriate query
                console.log('Successfully updated date channel information in the database.')
            ...
            // getting the updatedate command to execute right after channelcount has been set
            ...
            const updateDateCommand = serviceCommands.find(c => c.name === 'updatedate'); // get the updatedate command


            if (updateDateCommand) { // check if the command exists
                console.log('Handing execution to updatedate command.');
                updateDateCommand.execute(message, con, args); // execute the updatedate command
            }
            else {
                console.log('Command updatedate not found. Skipping attempted updating of newly created date channel.');
            }
        ...
        try { // send output embed with information about the command's success
            ...
            console.log(`Command setupdate, started by ${message.member!.user.tag}, terminated successfully in ${message.guild!.name}.`);
        } catch (e) {
