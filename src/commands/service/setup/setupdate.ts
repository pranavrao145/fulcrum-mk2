...
const command: ICommand = {
  ...
  help: "Sets up Fulcrum's date channel feature.",
  alias: ["sd", "sud"],
  syntax: "f!setupdate [voice channel role mention]",
  async execute(message: Message, con: Client, args?: string[]) {
    console.log(
      `Command setupdate started by user ${message.member!.user.tag} in guild ${
        message.guild!.name
      }.`
    );
    ...
    try {
      console.log(
        "Attempting to retrieve date channel information from the database."
      );
      ...
      try {
        ...
        console.log(
          "Successfully updated date channel information in the database."
        );
      ...
      if (updateDateCommand) {
        ...
        console.log(
          "Command updatedate not found. Skipping attempted updating of newly created date channel."
        );
      }
    } catch (e) {
      console.log(
