...
const command: ICommand = {
  ...
  async execute(message: Message, _con: Client, args?: string[]) {
    console.log(
      `Command renamechannel started by user ${
    ...
    if (textChannel) {
      ...
      try {
        ...
        console.log(
          `Failed to rename channel ${(textChannel as TextChannel).name}.`
        );
      ...
      try {
        ...
        console.log(
          `Failed to rename channel ${(voiceChannel as VoiceChannel).name}.`
        );
        outputEmbed.addField("Status", "Failed");
      }
    } else {
      ...
      console.log(
        `Argument given was invalid for a text channel mention or a voice channel role. Stopping execution.`
      );
      try {
        return await message.channel.send(
          "Invalid argument for channel or voice channel role."
        );
      } catch (e) {
        console.log(
          `There was an error sending a message in the guild ${
