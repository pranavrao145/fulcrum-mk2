...
const command: ICommand = {
    ...
    alias: ['cc'],
    syntax: 'f!createchannel [name (underscores for spaces)] (type, GUILD_TEXT/GUILD_VOICE, default GUILD_TEXT) (privacy, public/private, default public)',
        ...
        if (privacy) { // if there is a privacy, meaning there will be a type and a channelName as well
            ...
            if (typeFormatted !== 'GUILD_TEXT' && typeFormatted !== 'GUILD_VOICE') { // check if the type given was valid
                try {
                    ...
                    return await message.channel.send('Invalid value for type! Must be GUILD_TEXT or GUILD_VOICE.')
                } catch (e) {
                    console.log(`There was an error sending a message in the guild ${message.guild!.name}! The error message is below:`);
                    console.log(e);
                    return;
