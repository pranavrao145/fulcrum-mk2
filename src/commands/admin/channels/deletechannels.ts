    ...
    async execute(message: Message, _con: Client, args?: string[]) {
        ...
        for (const channelMention of args) { // iterate through each of the mentions
            ...
            if (textChannel) { // if a text channel is detected
                try { // attempt to delete the text channel right away
                    await timeout(300); // setting a short timeout to prevent abuse of Discord's API
                    await textChannel.delete(); // delete the channel
                    console.log(`Channel ${(textChannel as TextChannel).name} of type ${textChannel.type} deleted successfully.`)
                    outputEmbedText += `\n**${(textChannel as TextChannel).name}**: Channel deleted successfully.`;
                } catch (e) {
                    console.log(`Failed to delete channel ${(textChannel as TextChannel).name}.`)
                ...
                try { // attempt to delete the text channel right away
                    ...
                    console.log(`Channel ${(voiceChannel as VoiceChannel).name} of type ${voiceChannel!.type} deleted successfully.`)
                    outputEmbedText += `\n**${(voiceChannel as VoiceChannel).name}**: Channel deleted successfully.`;
                } catch (e) {
                    console.log(`Failed to delete channel ${(voiceChannel as VoiceChannel).name}.`)
                    outputEmbedText += `\n**${(voiceChannel as VoiceChannel).name}**: Couldn\'t delete channel.`;
                }

            } else { // if the argument supplied is neither a channel nor a role
                console.log(`Argument given was invalid for a text channel mention or a voice channel role. Skipping over it.`)
