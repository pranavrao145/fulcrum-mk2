...
const command: ICommand = {
  ...
  async execute(message: Message, _con: Client, _args?: string[]) {
    console.log(
      `Command lock started by user ${message.member!.user.tag} in guild ${
        message.guild!.name
      }.`
    );
    ...
    try {
      ...
      console.log(
        `Successfully locked ${(messageChannel as TextChannel).name}.`
      );
      outputEmbed.addField("Status", "Success");
    } catch (e) {
      console.log(`Failed to lock ${(messageChannel as TextChannel).name}.`);
