...
const command: ICommand = {
  ...
  help: "Unbans the given user. You can optionally specify a reason for unbanning.",
  alias: ["ub"],
  syntax: "f!unban [user ID] (reason)",
  async execute(message: Message, _con: Client, args?: string[]) {
    console.log(
      `Command unban started by user ${message.member!.user.tag} in guild ${
        message.guild!.name
      }.`
    );
    ...
    try {
      ...
      console.log(
        "User ID supplied was invalid or couldn't find ban. Stopping execution."
      );
      try {
        return await message.channel.send(
    ...
    if (reasonToUnban) {
      // check if the user has provided a reason to unban
      try {
