...
const command: ICommand = {
  ...
  async execute(message: Message, _con: Client, args?: string[]) {
    ...
    if (!message.member!.permissions.has("MANAGE_NICKNAMES")) {
      ...
      try {
        ...
        return await message.reply(
          "Sorry, you need to have the `MANAGE_NICKNAMES` permission to use this command."
        );
      } catch (e) {
        console.log(
    ...
    try {
      // attempt to set the user's nickname to the new nickname given
      await user.setNickname(newNick);
      console.log(
        `Successfully changed ${user!.user.tag}'s nickname to ${
          user.nickname
        }.'`
      );
    ...
    try {
      ...
      console.log(
        `Command changenickname, started by ${
          message.member!.user.tag
