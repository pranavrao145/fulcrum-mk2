...
const command: ICommand = {
  ...
  async execute(message: Message, _con: Client, args?: string[]) {
    console.log(
      `Command changevoicepermissions started by user ${
    ...
    for (const permissionChange of args) {
      ...
      if (operation !== "r") {
        ...
        if (isNaN(permissionNum)) {
          ...
          if (
            !voiceChannelPermissions.includes(
          ...
          if (
            permissionNum < 1 ||
            permissionNum > voiceChannelPermissions.length
          ) {
        ...
        switch (
          ...
          case "+":
            try {
              ...
              await (voiceChannel as VoiceChannel).permissionOverwrites.create(
                role,
                voiceChannelPermissionsEnable.get(
          ...
          case "-":
            try {
              ...
              await (voiceChannel as VoiceChannel).permissionOverwrites.create(
                role,
                voiceChannelPermissionsDisable.get(
                  permission as PermissionResolvable
                )
    ...
    try {
      ...
      console.log(
        `Command changevoicepermissions, started by ${
          message.member!.user.tag
