...
import {
  ...
  stageVoiceChannelPermissionsEnable,
  stageVoiceChannelPermissionsDisable,
} from "../../../utils/information";

const command: ICommand = {
  name: "changestagepermissions",
  description:
    "Changes the given role's permissions in a voice channel according to the changes given. Permissions are referred to by their name or by their number (see f!listpermissions). You can change permissions by specifiying an operation and a permission. Operation can be + for add, - for remove, or just r (with nothing after) for resetting permissions. E.g. to allow CREATE_INSTANT_INVITE and MUTE_MEMBERS on a role for a certain voice channel, simply give the command: f!changestagepermissions @role @voice-channel-role +CREATE_INSTANT_INVITE +MUTE_MEMBERS. Alternatively, if you prefer to use numbers, you can give the command as: f!changestagepermissions @role @voice-channel-role +1 +8",
  alias: ["csp", "changestageperm "],
  syntax:
    "f!changestagepermissions [role mention or number] [stage channel role mention] [permission changes, (+/-/r)(permission number)]",
  async execute(message: Message, _con: Client, args?: string[]) {
    console.log(
      `Command changestagepermissions started by user ${
    ...
    const outputEmbed = new MessageEmbed() // create a new embed for output
      .setColor("#FFFCF4")
      .setTitle("Change Stage Permissions - Report");
    ...
    const stageVcRoleMention = args.shift(); // get the voice channel role mention
    ...
    if (isNaN(parseInt(roleMention!))) {
      ...
      if (roleMention === "@everyone" || roleMention == "everyone") {
    ...
    const stageVcRole = getRoleFromMention(message, stageVcRoleMention!); // get the actual role mention

    if (!stageVcRole) {
      // if the stage voice channel role does not exist
      console.log(
        "Invalid stage voice channel role given. Stopping execution."
      );
      try {
        return await message.channel.send("Invalid role.");
      } catch (e) {
    ...
    const voiceChannel = message
      ...
      .find((c) => c.name === stageVcRole.name); // attempt to get stage voice channel with the same name

    if (!voiceChannel) {
      console.log(
        "No stage voice channel found associated with the role supplied. Stopping execution."
      );
      try {
        return await message.channel.send(
          "No stage voice channel found for that role!"
        );
      } catch (e) {
        console.log(
          `There was an error sending a message in the guild ${
    ...
    for (const permissionChange of args) {
      ...
      if (operation !== "r") {
        ...
        console.log(
          "Attempting to find permission number in voice channel permissions."
        );
        ...
        if (isNaN(permissionNum)) {
          ...
          if (
            !stageVoiceChannelPermissions.includes(
          ...
          if (
            permissionNum < 1 ||
            permissionNum > stageVoiceChannelPermissions.length
          ) {
          ...
          permission = stageVoiceChannelPermissions[permissionNum - 1]; // get the permission name from the list using index
        }
        ...
        switch (
          ...
          case "+":
            try {
              ...
              await (voiceChannel as StageChannel).permissionOverwrites.create(
                role,
                stageVoiceChannelPermissionsEnable.get(
                  permission as PermissionResolvable
                )
              ); // add the permission given
              console.log(
          ...
          case "-":
            try {
              ...
              await (voiceChannel as StageChannel).permissionOverwrites.create(
                role,
                stageVoiceChannelPermissionsDisable.get(
                  permission as PermissionResolvable
                )
              ); // remove the permission given
              console.log(
        ...
        try {
          ...
          const currentOverwrites = (
            voiceChannel as StageChannel
          ).permissionOverwrites.cache.get(role.id); // get the current permissions for the role
          if (currentOverwrites) {
            // only act if the permission overwrites actually exist
    ...
    try {
      ...
      if (outputEmbedText !== "") {
        ...
        outputEmbed.setDescription(
          `**Command executed by:** ${
            message.member!.user.tag
          }\n**Modified perimssions of role:** ${
            role.name
          }\n**Modified role's permissions in:** ${
            (voiceChannel as StageChannel).name
          }`
        );
        await message.channel.send({ embeds: [outputEmbed] });
      }
      console.log(
        `Command changestagepermissions, started by ${
          message.member!.user.tag
