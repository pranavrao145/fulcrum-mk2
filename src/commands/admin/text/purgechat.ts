...
const command: ICommand = {
  ...
  async execute(message: Message, _con: Client, args?: string[]) {
    console.log(
      `Command purgechat by user ${message.member!.user.tag} in guild ${
        message.guild!.name
      }.`
    );
    ...
    if (!message.member!.permissions.has("MANAGE_MESSAGES")) {
      ...
      try {
        ...
        return await message.reply(
          "Sorry, you need to have the `MANAGE_MESSAGES` permission to use this command."
        );
      } catch (e) {
        console.log(
    ...
    if (isNaN(parsedNum)) {
      // check if the number given is not a valid number.
      try {
        console.log(
          "Invalid value given for number of messages. Stopping execution."
        );
        return await message.channel.send(
          "Invalid number. Must be a number from 2 to 100."
        );
      } catch (e) {
        console.log(
          `There was an error sending a message in the guild ${
    ...
    if (message.channel.type !== "GUILD_TEXT") {
      // check if the channel type is actually a text channel (or can't bulk delete)
      try {
        console.log("Invalid channel type for purge chat. Stopping execution.");
        return await message.channel.send(
          "Can't use purgechat in this type of channel!"
        );
      } catch (e) {
        console.log(
          `There was an error sending a message in the guild ${
    ...
    try {
      ...
      if (outputEmbed.fields.length > 0) {
        ...
        const outputEmbedMessage = await message.channel.send({
          embeds: [outputEmbed],
        }); // keep track of the message with the embed for deletion
        await timeout(5000); // wait 5 seconds
        await outputEmbedMessage.delete(); // delete output embed message
      }
      console.log(
        `Command purgechat, started by ${
