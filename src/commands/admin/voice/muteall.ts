...
const command: ICommand = {
  name: "muteall",
  description:
    "Mutes all the members of the specified voice channel. Note: when used in a stage channel, this command will only affect speakers.",
  alias: ["ma"],
  syntax: "f!muteall [voice channel role mention]",
  async execute(message: Message, _con: Client, args?: string[]) {
    console.log(
      `Command muteall started by user ${message.member!.user.tag} in guild ${
        message.guild!.name
      }.`
    );
    ...
    for (const member of vcMembers) {
      ...
      try {
        ...
        console.log(
          `Member ${member.user.tag} muted successfully in voice channel ${voiceChannel.name}`
        );
      } catch (e) {
        console.log(
          `Failed to mute member ${member.user.tag} in voice channel ${voiceChannel.name}`
        );
    ...
    if (overallSuccess) {
      ...
      outputEmbed.addField(
        "Status",
        "Failed - some members may not have been muted"
      );
    }
