...
const command: ICommand = {
    name: 'removefromuser',
    description: 'Removes the given roles from the user given. You can remove upto 10 roles from a user with one command.',
    alias: ['rfu', 'ru'],
    ...
    async execute(message: Message, _con: Client, args?: string[]) {
        ...
        try { // send output embed with information about the command's success
            ...
            console.log(`Command removefromuser, started by ${message.member!.user.tag}, terminated successfully in ${message.guild!.name}.`);
        } catch (e) {
