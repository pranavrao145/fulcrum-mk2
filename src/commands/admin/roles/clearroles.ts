...
const command: ICommand = {
  slashCommand: new SlashCommandBuilder()
    .setName("clearroles")
    .setDescription(
      "Removes the given role from all users that have it."
    ),
  help: "Removes the given role from all users that have it. You can clear upto 10 roles with one command.",
  alias: ["clr"],
  syntax: "f!clearroles [role mentions or numbers (10 max)]",
  async execute(message: Message, _con: Client, args?: string[]) {
    console.log(
      `Command clearroles started by user ${
    ...
    for (const mention of args!) {
      ...
      if (!members) {
        // check if the role members actually exist
        console.log(
          "A role supplied did not have any members. Skipping over it."
        );
      ...
      for (const member of members) {
        // iterate through the members that have the role
        if (!member) {
          console.log(
            "A member with the role did not exists. Skipping over them."
          );
        ...
        try {
          ...
          console.log(
            `Role ${role.name} was removed from user ${member.user.tag} successfully.`
          );
        } catch (e) {
