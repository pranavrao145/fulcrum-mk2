...
const command: ICommand = {
  ...
  async execute(message: Message, _con: Client, args?: string[]) {
    console.log(
      `Command createrole started by user ${
    ...
    if (roleName!.startsWith("@")) {
      // check if the role starts with an @ and get rid of it if so
    ...
    if (
      getRoleFromMention(message, roleName!) ||
      message.guild!.roles.cache.find((r) => r.name === roleName) ||
      getChannelFromMention(message, roleName!) ||
      getUserFromMention(message, roleName!)
    ) {
      // checking to see if the role already exists on the server as a role or anything else
      console.log(
        "Invalid role name or role already exists in server. Stopping execution."
      );
      outputEmbed.addField(
        `${roleName}`,
        "Invalid role name or role already exists on this server."
      );
    ...
    if (roleColor) {
      // in the case there is a role colour specified
      console.log(
        "Role colour detected. Attempting to create role with colour."
      );
      ...
      if (!isValidColor(roleColor!)) {
        // check if the color supplied was valid
      ...
      try {
        ...
        console.log(
          `Role ${roleName} created successfully in ${message.guild!.name}.`
        );
      } catch (e) {
        outputEmbed.addField("Status", "Failed");
        console.log(
          `Failed to create role ${roleName} in server ${
            message.guild!.name
          }. The error message is below:`
        );
      }
      ...
      try {
        console.log(
          "No role colour detected. Attempting to create role without colour."
        );
        await message.guild!.roles.create({
          // create the role with the needed data
          name: roleName,
